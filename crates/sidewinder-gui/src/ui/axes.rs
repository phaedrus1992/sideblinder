...
use sidewinder_app::config::{AxisConfig, ResponseCurve};
...
const SMOOTHING_MAX: f32 = sidewinder_app::config::SMOOTHING_MAX as f32;
...
///
/// Mirrors `ResponseCurve::apply` from `sidewinder-app/config.rs` without the
...
mod tests {
    ...
    use sidewinder_app::config::ResponseCurve;
