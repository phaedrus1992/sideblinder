//! `PipeBackend`: reads live `GuiFrame`s from a running `sidewinder-app` instance
//! via the named pipe `\\.\pipe\SidewinderGui`.
//!
...
/// Live-data backend that reads [`GuiFrame`]s from the `sidewinder-app` IPC pipe.
...
impl PipeBackend {
    /// Try to connect to a running `sidewinder-app` instance.
    ///
    /// Opens `\\.\pipe\SidewinderGui` and spawns a background thread that reads
...
mod windows_impl {
    ...
    use sidewinder_ipc::FRAME_TOTAL_LEN;
    use std::sync::mpsc;
    ...
    pub(super) fn connect() -> Result<PipeBackend, BackendError> {
        let pipe_name_wide: Vec<u16> = sidewinder_ipc::PIPE_NAME
            .encode_utf16()
        ...
        if handle == INVALID_HANDLE_VALUE {
            return Err(BackendError::PipeConnect(
                "sidewinder-app pipe not available — is the app running?".to_owned(),
            ));
        }
