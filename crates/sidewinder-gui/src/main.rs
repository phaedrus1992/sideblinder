...
//!
//! On startup the app tries to connect to a running `sidewinder-app` instance
//! via the named pipe [`sidewinder_ipc::PIPE_NAME`].  If the pipe is not
...
fn main() -> ExitCode {
    ...
    tracing::info!("sidewinder-gui starting");

    let config_path = sidewinder_app::config::default_config_path();

    // Attempt to attach to a running sidewinder-app via the named pipe.
    ...
    let backend: Box<dyn backend::GuiBackend> =
        match pipe_backend::PipeBackend::connect() {
            Ok(b) => {
                tracing::info!("attached to running sidewinder-app via IPC pipe");
                Box::new(b)
            }
            Err(e) => {
