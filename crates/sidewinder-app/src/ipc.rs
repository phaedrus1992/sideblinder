...
impl InputSnapshot {
    /// Build from a [`sidewinder_hid::input::InputState`].
    /// Called by the Windows bridge and in tests.
    #[cfg(test)]
    #[must_use]
    pub fn from_input_state(state: &sidewinder_hid::input::InputState) -> Self {
        Self {
            axes: [state.axes[0], state.axes[1], state.axes[2], state.axes[3]],
            buttons: u32::from(state.buttons),
            pov: match state.pov {
                sidewinder_hid::input::PovDirection::North => 0,
                sidewinder_hid::input::PovDirection::NorthEast => 1,
                sidewinder_hid::input::PovDirection::East => 2,
                sidewinder_hid::input::PovDirection::SouthEast => 3,
                sidewinder_hid::input::PovDirection::South => 4,
                sidewinder_hid::input::PovDirection::SouthWest => 5,
                sidewinder_hid::input::PovDirection::West => 6,
                sidewinder_hid::input::PovDirection::NorthWest => 7,
                sidewinder_hid::input::PovDirection::Center => 0xFF,
            },
        }
    }
}
...
mod windows_impl {
    ...
    const DEVICE_SYMLINK: &str = "\\\\.\\SidewinderFFB2";
