//! Named-pipe server that broadcasts `GuiFrame` snapshots to `sidewinder-gui`.
//!
//! Spawns a background tokio task that creates `\\.\pipe\SidewinderGui`, accepts
//! one client at a time, and streams a 26-byte framed [`sidewinder_ipc::GuiFrame`]
...
/// Build a [`sidewinder_ipc::GuiFrame`] from the current watch-channel state.
///
...
/// Spawn a background tokio task that streams [`sidewinder_ipc::GuiFrame`]s
/// to a connected GUI client at ~30 Hz.
///
/// The task creates `\\.\pipe\SidewinderGui` and waits for one client at a
    ...
    tokio::spawn(async move {
        ...
        loop {
            let mut server = match ServerOptions::new()
                ...
                .create(sidewinder_ipc::PIPE_NAME)
            {
                Ok(s) => s,
                Err(e) => {
                    tracing::warn!(
