...
//! # Config subcommands
//!
//! `sidewinder-app config --validate [--config <path>]` — validate a config file.
//! `sidewinder-app config --generate [--config <path>]` — write a documented default config.
...
fn spawn_input_only_status_poller(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let next = match tokio::task::spawn_blocking(SidewinderDevice::open).await {
...
async fn main() -> ExitCode {
    ...
    let (status_rx, startup) = {
        ...
        use sidewinder_hid::{device::SidewinderDevice, input::InputState};
        use std::sync::Arc;
        use tokio::sync::watch;
        ...
        let (status_rx, input_state_rx) = match (SidewinderDevice::open(), driver_result) {
            (Ok(device), Ok(driver_ipc)) => {
                info!("physical device and driver IPC opened — starting full bridge");
                let transport: Arc<dyn sidewinder_hid::hid_transport::HidTransport> =
        ...
        // Stream GuiFrame snapshots to any connected sidewinder-gui instance.
