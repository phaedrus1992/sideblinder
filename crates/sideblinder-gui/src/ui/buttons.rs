...
use sideblinder_app::config::{BUTTON_COUNT, BUTTON_COUNT_U8};
                ...
                && let Some(phys) = state.selected_phys.take()
            {
                ...
                match config_writer::patch_u8(ctx.config_path, &key, virt_u8) {
                    ...
                    Ok(()) => {
                        ...
                        match sideblinder_app::config::Config::load(ctx.config_path) {
...
/// Mirrors `Config::validate_hat` in sideblinder-app which warns on idx >= 32.
