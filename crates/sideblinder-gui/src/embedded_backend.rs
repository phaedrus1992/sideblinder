...
//!
//! Used as a fallback when `sideblinder-app` is not running and the named pipe
...
fn start_impl(config_path: PathBuf) -> Result<EmbeddedBackend, BackendError> {
    ...
    let device = SideblinderDevice::open().map_err(|e| BackendError::DeviceOpen(e.to_string()))?;
...
/// Build a [`GuiFrame`] from raw [`sideblinder_hid::input::InputState`] and config scalars.
///
/// Duplicates the logic in `sideblinder-app/bridge.rs` so the GUI does not need
/// to import platform-gated bridge internals.
#[cfg(target_os = "windows")]
fn frame_from_state(
    state: &sideblinder_hid::input::InputState,
    connected: bool,
    ffb_enabled: bool,
    ffb_gain: u8,
) -> GuiFrame {
...
fn device_read_loop(
    device: sideblinder_hid::device::SideblinderDevice,
    config_path: PathBuf,
    tx: mpsc::SyncSender<GuiFrame>,
    alive: Arc<AtomicBool>,
) {
    let mut config = sideblinder_app::config::Config::load(&config_path).unwrap_or_default();
    let mut last_reload = std::time::Instant::now();

    loop {
        // Reload config periodically so GUI write-backs are reflected.
        if last_reload.elapsed() >= CONFIG_RELOAD_INTERVAL {
            match sideblinder_app::config::Config::load(&config_path) {
