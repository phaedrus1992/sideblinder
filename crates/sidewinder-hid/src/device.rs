...
//!
//! [`SidewinderDevice`] owns a [`HidTransport`] and exposes a clean interface
...
/// Error returned by [`SidewinderDevice`] operations.
#[derive(Debug, Error)]
...
impl SidewinderDevice {
    /// Wrap an existing transport.  Prefer [`SidewinderDevice::open`] in
    ...
    pub fn open() -> Result<Self, DeviceError> {
        use crate::enumerate::find_sidewinder;
        use crate::hid_transport::WindowsHidTransport;

        let info = find_sidewinder()
...
mod tests {
    ...
    fn make_device() -> (SidewinderDevice, Arc<MockTransport>) {
        let mock = Arc::new(MockTransport::new());
        let transport = mock.clone();
        let device = SidewinderDevice::from_transport(Box::new(MockWrapper(transport)));
        (device, mock)
    }
