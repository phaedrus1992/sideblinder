...
/// Return the platform-default config file path used by `sidewinder-app`.
///
/// SYNC: This must stay identical to `sidewinder_app::config::default_config_path`.
...
enum Command {
    ...
    Replay {
        /// Input file path produced by `sidewinder-diag capture`.
        file: std::path::PathBuf,
    },
...
/// Run hardware diagnostics, write results to `%TEMP%\sidewinder-diag.log`,
...
fn run_diagnose() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let log_path = std::env::temp_dir().join("sidewinder-diag.log");
...
fn build_diagnostic_report() -> String {
    use sidewinder_hid::enumerate::enumerate_hid_devices;
    ...
    match enumerate_hid_devices() {
        ...
        Ok(devices) => {
            if devices.is_empty() {
                ...
                for d in &devices {
                    let marker = if d.is_sidewinder_ff2() {
                        " <-- SIDEWINDER FF2"
                    } else {
                        ""
                    };
            ...
            if let Some(sw) = devices.iter().find(|d| d.is_sidewinder_ff2()) {
...
fn probe_sidewinder_device(
    out: &mut String,
    sw: &sidewinder_hid::enumerate::HidDeviceInfo,
) -> Result<(), ()> {
    use std::fmt::Write as _;
...
/// Try to open `\\.\SidewinderFFB2` (the kernel device created by the driver)
...
fn poll_task_windows(state: &Arc<Mutex<DiagState>>) {
    use sidewinder_hid::device::SidewinderDevice;
    use sidewinder_hid::enumerate::find_sidewinder;
    use sidewinder_hid::input::PovDirection;

    match SidewinderDevice::open() {
        Ok(dev) => {
            // Set device info once after a successful open.
            let info_str = find_sidewinder()
                .ok()
                .flatten()
...
mod tests {
    ...
    fn test_pov_to_nibble_all_variants() {
        use sidewinder_hid::input::{InputState, PovDirection, parse_input_report};
