//! Inter-process communication protocol between `sideblinder-app` and `sideblinder-gui`.
//!
//! The protocol is a simple length-prefixed binary framing over a Windows named pipe.
//! `sideblinder-app` acts as the server; `sideblinder-gui` connects as the client.
//!
...
/// A live-state snapshot sent from `sideblinder-app` to `sideblinder-gui` at ~30 Hz.
///
