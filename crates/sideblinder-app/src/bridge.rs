...
//!
//! 1. **Input loop** — polls the physical Sidewinder via `SideblinderDevice`,
...
/// Build a [`sideblinder_ipc::GuiFrame`] from a raw [`InputState`] and config scalars.
///
...
#[cfg(target_os = "windows")]
pub struct Bridge {
    transport: std::sync::Arc<dyn sideblinder_hid::hid_transport::HidTransport>,
...
/// retries `SideblinderDevice::open` every [`RECONNECT_INTERVAL`].  Once the
...
async fn input_loop(
    ...
    transport_tx: tokio::sync::watch::Sender<
        std::sync::Arc<dyn sideblinder_hid::hid_transport::HidTransport>,
    >,
    ipc: std::sync::Arc<dyn crate::ipc::DriverIpc>,
    ...
    loop {
        ...
        let report = match raw {
            ...
            Ok(Err(e)) => {
                ...
                // `SideblinderDevice::open` runs in `spawn_blocking` to avoid
                ...
                loop {
                    ...
                    let open_result = tokio::task::spawn_blocking(SideblinderDevice::open).await;
                    match open_result {
                        Ok(Ok(device)) => {
                            let new_transport: std::sync::Arc<
                                dyn sideblinder_hid::hid_transport::HidTransport,
        ...
        let mut state = match sideblinder_hid::input::parse_input_report(&report) {
            Ok(s) => s,
            Err(e) => {
                warn!(
                internal_error = %e,
...
async fn ffb_loop(
    ...
    use sideblinder_hid::ffb::{FfbOperation, REPORT_ID_DEVICE_GAIN, build_operation_report};
...
mod tests {
    ...
    /// fields correctly into a [`sideblinder_ipc::GuiFrame`].
    #[test]
