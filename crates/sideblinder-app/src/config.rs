...
//!
//! The config file lives at `%APPDATA%\Sideblinder\config.toml` on Windows, or
//! `~/.config/sideblinder/config.toml` elsewhere.  The app watches the file for
...
///
/// Written by the `sideblinder-diag calibrate` wizard and read by the bridge
...
impl HatConfig {
    ...
    #[cfg(any(target_os = "windows", test))]
    #[must_use]
    pub fn button_for(&self, pov: sideblinder_hid::input::PovDirection) -> Option<u8> {
            ...
            if min >= max {
                warnings.push(format!(
                    "[{name}] min ({min}) must be less than max ({max}) — \
                     calibration range is degenerate; run sideblinder-diag calibrate"
                ));
            }
        }
    }
...
# Generated by `sideblinder-app config --generate`.
...
# ── Calibration ──────────────────────────────────────────────────────────────────
# Physical axis ranges measured by `sideblinder-diag calibrate`.
# The app stretches these values to the full virtual range.
# Run `sideblinder-diag calibrate` to measure your hardware automatically.
[calibration]
x_min = -32768
x_max = 32767
y_min = -32768
...
/// Run `sideblinder-app config --validate`: load the config file and print a
...
/// Run `sideblinder-app config --generate`: write a documented default config
...
///
/// SYNC: A copy of this function lives in `sideblinder_diag::main`.
...
mod tests {
    ...
    fn default_toml_file_parses() {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent() // crates/sideblinder-app → crates
    ...
    fn generate_toml_has_comments() {
        let toml = Config::generate_toml();
        assert!(toml.contains("# Sidewinder Force Feedback 2"), "must have header comment");
        assert!(
