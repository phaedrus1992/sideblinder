...
pub const FF2_VID: u16 = 0x045E;
pub const FF2_PID: u16 = 0x001B;
...
impl HidDeviceInfo {
    ...
    #[must_use]
    pub fn is_ff2(&self) -> bool {
        self.vendor_id == FF2_VID && self.product_id == FF2_PID
    }
}
...
pub fn find_sideblinder() -> Result<Option<HidDeviceInfo>, String> {
    Ok(enumerate_hid_devices()?
        .into_iter()
        .find(HidDeviceInfo::is_ff2))
}
...
mod tests {
    ...
    fn find_sideblinder_none_on_non_windows() {
        ...
        assert!(
            result
                .expect("find_sideblinder should succeed on non-Windows")
                .is_none()
        );
    }
